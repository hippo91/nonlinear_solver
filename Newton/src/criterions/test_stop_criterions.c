...
int main() {
    ...
    bool success = true;
    if (!assert_equal_arrays(x_k_pun, expected_x_k_un, PB_SIZE, "x_k_pun")) success = false;
    if (!assert_equal_bool_arrays(has_converged, expected_has_converged, PB_SIZE, "has_converged")) success = false;
